...
async def search(bot, message):
    ...
    if message.text.startswith("/"):
       return    
    query   = message.text 
    head    = "<u>𝐇𝐞𝐫𝐞 i𝐬 𝐭𝐡𝐞 𝐫𝐞𝐬𝐮𝐥𝐭𝐬 🤤👇\n\n 𝙼𝚊𝚒𝚗𝚝𝚊𝚒𝚗𝚎𝚍 𝙱𝚢 ❥ </u> <b><I>@NischayYdv</I></b>\n\n"
    results = ""
    try:
       for channel in channels:
           async for msg in User.search_messages(chat_id=channel, query=query):
               ...
               if name in results:
                  continue 
               results += f"<b><I>📁 {name}\n🔗 {msg.link}</I></b>\n\n 𝙼𝚊𝚒𝚗𝚝𝚊𝚒𝚗𝚎𝚍 𝙱𝚢 : NischayYdv"                                                      
       if bool(results)==False:
          movies = await search_imdb(query)
          buttons = []
          for movie in movies: 
              buttons.append([InlineKeyboardButton(movie['title'], callback_data=f"recheck_{movie['id']}")])
          msg = await message.reply_photo(photo="https://graph.org/file/53d67940f31322c3f5dbe.jpg",
                                          caption="<b><I>😓 I Could not find anything related to Your Query😕.\nDid you mean any of these?</I></b>", 
